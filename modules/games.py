...
GAMES = {
    ...
    "7daystodie": {
        ...
        "name": "7 Days to Die",
        "category": "Supervivencia",
        "description": "Busca supervivientes para aguantar hordas."
    },
...
def usuario_puede_controlar_foxy(usuario):
    """
    Permite controlar a Foxy a:
    - Administradores
    - Mod
    - Cualquier rol situado por encima de Mod
    """
    ...
    guild = getattr(usuario, "guild", None)
    ...
    for rol in guild.roles:
        ...
        nombre = rol.name.strip().lower()

        if nombre == "mod" or nombre.startswith("mod "):
            rol_mod = rol
            break
    ...
    # El usuario debe tener un rol igual o superior
    # a la posición de Mod.
    for rol in getattr(usuario, "roles", []):

        if rol.position >= rol_mod.position:
            return True
...
def crear_embed(busqueda):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Los **Mods o superiores** pueden controlar a 🦊 Foxy."
        ),
...
class BusquedaView(discord.ui.View):
    ...
    async def unirme(self, interaction, button):
        ...
        for jugador in jugadores:
            ...
            if not es_foxy(jugador) and jugador.id == interaction.user.id:
    ...
    @discord.ui.button(
        label="Invitar a Foxy",
        emoji="🦊",
        style=discord.ButtonStyle.primary
    )
    async def invitar_foxy(self, interaction, button):

        # ----------------------------------------------------
        # COMPROBAR PERMISOS
        # ----------------------------------------------------

        if not usuario_puede_controlar_foxy(interaction.user):

            await interaction.response.send_message(
                "❌ Solo los **Mods o superiores** "
                "pueden invitar o sacar a Foxy.",
                ephemeral=True
            )
        ...
        # ----------------------------------------------------
        # SI FOXY NO ESTÁ -> INVITARLO
        # ----------------------------------------------------

        if not foxy_puede_unirse(
            jugadores,
            MAX_JUGADORES
        ):

            await interaction.response.send_message(
                "❌ No se puede invitar a Foxy porque "
    ...
    async def salir(self, interaction, button):
        ...
        jugador = next(
            (
                ...
                for jugador in jugadores
                if not es_foxy(jugador)
                and jugador.id == interaction.user.id
            ),
            None
        )
    ...
    async def cerrar(self, interaction, button):
        ...
        if interaction.user.id != busqueda["organizador"].id:
            ...
            await interaction.response.send_message(
                "❌ Solo el organizador puede cerrar "
                "la búsqueda.",
                ephemeral=True
            )
...
class ElegirJuegoView(discord.ui.View):
    ...
    async def crear_busqueda(self, interaction, juego):
        ...
        if interaction.user.id != self.autor_id:
            ...
            await interaction.response.send_message(
                "❌ Solo quien creó el menú "
                "puede elegir el juego.",
                ephemeral=True
            )

            return

        # Comprobar si ya tiene búsqueda activa
